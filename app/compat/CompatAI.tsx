...
const AlphabetScroller = ({ letters, onLetterClick, onScrollToTop }: AlphabetScrollerProps) => {
    return (
        <div className="fixed right-0 top-1/2 transform -translate-y-1/2 z-40 flex flex-col items-center bg-white/50 p-2 rounded-l-lg backdrop-blur-sm h-auto max-h-[80vh] justify-around">
            <button
...
export default function App() {
  ...
  if (loadingData) {
    return (
      <div className="min-h-screen text-[var(--text-dark)] flex items-center justify-center p-4 font-sans antialiased water-bg">
        <div className="text-center">
          <div className="animate-spin inline-block w-16 h-16 border-4 border-[var(--accent-primary)] border-t-transparent rounded-full"></div>
          <p className="mt-4 text-xl">Loading fish data...</p>
        </div>
      </div>
    );
  }
  ...
  if (error && !localFishData) {
    return (
      <div className="min-h-screen text-red-800 flex items-center justify-center p-4 font-sans antialiased water-bg">
        <div className="text-center p-8 bg-red-100 border border-red-300 rounded-lg shadow-inner max-w-md">
    ...
    <div className="min-h-screen text-[var(--text-dark)] font-sans antialiased animate-fade-in water-bg">
        <div className="p-4 sm:p-8 pb-12">
            ...
            <header className="text-center mb-6">
                ...
                <p className="mt-2 text-lg sm:text-xl text-gray-600">Select fish to get a compatibility report.</p>
            </header>
            ...
            <main className="max-w-full mx-auto bg-white/50 backdrop-blur-sm rounded-3xl p-4 sm:p-8 shadow-2xl overflow-hidden border border-[var(--border-color)]">
                {report && !showReport && (
                <div className="fixed top-4 right-4 z-50">
                    <button
                    onClick={() => setShowReport(true)}
                    disabled={loadingReport}
                    className={`flex items-center justify-center px-6 py-3 bg-white/60 backdrop-blur-sm hover:bg-white text-[var(--text-dark)] font-bold text-lg rounded-full shadow-lg transition-colors duration-300 transform hover:scale-105 border border-gray-300 ${loadingReport ? 'cursor-not-allowed opacity-50' : ''}`}
                    >
                ...
                {localFishData[selectedCategory].map((fish: any, index: number) => {
                    ...
                    className={`flex flex-col rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-xl animate-scale-in border ${
                        selectedFish.some(f => f.name === fish.name)
                        ? "bg-white ring-4 ring-[var(--accent-primary)] border-transparent"
                        : "bg-white/70 border-[var(--border-color)] hover:bg-white"
                    ...
                    <div className="p-2 flex-grow flex flex-col justify-center">
                        ...
                        <p className="text-xs sm:text-sm text-gray-600 italic text-center">{fish.commonNames.join(', ')}</p>
                    </div>
                    </div>
                )})}
                </div>
                ...
                <div className="max-w-full mx-auto flex flex-col sm:flex-row justify-center sm:justify-between items-center gap-4">
                    ...
                    <div className="flex items-center justify-center gap-4 w-full sm:w-auto order-2 sm:order-1">
                        <button
                            ...
                            disabled={loadingReport}
                            className="w-full sm:w-auto px-6 py-3 bg-[var(--accent-primary)] text-white font-bold text-lg rounded-full shadow-lg hover:bg-[#15a1b8] transition-colors duration-300 disabled:bg-gray-500 disabled:cursor-not-allowed transform hover:scale-105 flex items-center justify-center"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <button
                            ...
                            disabled={loadingReport}
                            className="w-full sm:w-auto px-6 py-3 bg-gray-600 text-white font-bold text-lg rounded-full shadow-lg hover:bg-gray-700 transition-colors duration-300 disabled:bg-gray-500 disabled:cursor-not-allowed transform hover:scale-105 flex items-center justify-center"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    {/* Selected Fish Tray */}
                    <div className="flex items-center gap-2 p-2 bg-white/60 backdrop-blur-md rounded-full shadow-lg order-1 sm:order-2 flex-wrap justify-center">
                        {selectedFish.map((fish, index) => (
                            <img
                                ...
                                alt={fish.name}
                                title={fish.name}
                                className="w-12 h-12 rounded-full object-cover border-2 border-[var(--accent-primary)] transition-all duration-300 ease-in-out transform hover:scale-110"
                                onError={(e: any) => {
                                    e.target.onerror = null;
        ...
        {loadingReport && (
            <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm">
                ...
                <button
                    ...
                    className="mt-4 px-6 py-3 bg-gray-600 text-white font-bold text-lg rounded-full shadow-lg hover:bg-gray-700 transition-colors duration-300"
                >
                    Cancel
                </button>
            </div>
        ...
        {showReport && (
            ...
            <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-full max-h-[90vh] bg-white text-[var(--text-dark)] rounded-3xl shadow-2xl flex flex-col animate-fade-in-down">
                <div className="relative flex-shrink-0 px-6 pt-4 pb-2 text-center z-10 border-b border-[var(--border-color)]">
                    <h3 className="text-2xl md:text-3xl font-extrabold text-[var(--text-dark)]">
                    ...
                    <button
                        ...
                        className="absolute top-2 right-2 sm:top-4 sm:right-4 text-gray-400 hover:text-gray-800 transition-colors duration-200 p-2"
                    >
                ...
                {error && (
                    <div className="text-center p-4 mb-4 bg-red-100 text-red-800 rounded-lg shadow-inner">
                ...
                {report && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-1 gap-6 mb-6">
                        <div className="bg-gray-50 border border-[var(--border-color)] p-6 rounded-2xl text-center">
                        ...
                        <p className="text-sm text-gray-600 mt-2">
                            {report.groupHarmonyScore < 0.5
                                ...
                                : "This score represents the compatibility of the most challenging pair in the group. A high score indicates that all selected fish are likely to coexist peacefully."}
                            <br/><br/>
                    ...
                    <div className="space-y-6 text-gray-700">
                        ...
                        <h4 className="text-2xl font-bold text-[var(--text-dark)] mb-2">Detailed Summary</h4>
                        <p className="text-lg leading-relaxed">{report.detailedSummary}</p>
                        </div>

                        {report.tankSize && (
                        <div className="border-t-2 border-[var(--border-color)] pt-6">
                            <h4 className="text-2xl font-bold text-[var(--text-dark)] mb-2">Recommended Tank Size</h4>
                            <p className="text-lg leading-relaxed">{report.tankSize}</p>
                        </div>
                        )}

                        {report.decorations && (
                        <div className="border-t-2 border-[var(--border-color)] pt-6">
                            <h4 className="text-2xl font-bold text-[var(--text-dark)] mb-2">Decorations and Setup</h4>
                            <p className="text-lg leading-relaxed">{report.decorations}</p>
                        </div>
                        )}

                        {report.careGuide && (
                        <div className="border-t-2 border-[var(--border-color)] pt-6">
                            <h4 className="text-2xl font-bold text-[var(--text-dark)] mb-2">Care Guide</h4>
                            <p className="text-lg leading-relaxed">{report.careGuide}</p>
                        </div>
                        )}

                        {report.compatibleFish && report.compatibleFish.length > 0 && (
                        <div className="border-t-2 border-[var(--border-color)] pt-6">
                            <h4 className="text-2xl font-bold text-[var(--text-dark)] mb-4">Compatible Tank Mates for Your Selection</h4>
                            ...
                            {selectedFish.map((fish: any) => (
                                ...
                                <img
                                    src={fish.imageURL}
                                    alt={fish.name}
                                    className="w-20 h-20 rounded-full border-2 border-[var(--border-color)] object-cover mb-2"
                                    onError={(e: any) => { e.target.onerror = null; e.target.src = "https://placehold.co/64x64/4B5563/F9FAFB?text=No+Image"; }}
                                />
                                <span className="font-semibold text-center text-sm text-[var(--text-dark)]">{fish.name}</span>
                                </div>
                            ))}
                            </div>
                            <ul className="list-disc list-inside columns-2 sm:columns-3 lg:columns-4">
                        ...
                        <div className="border-t-2 border-[var(--border-color)] pt-6">
                            ...
                            <div className="bg-gray-50 border border-[var(--border-color)] p-4 sm:p-6 rounded-2xl space-y-4">
                            <div>
                                <h5 className="font-semibold text-lg text-blue-600">Pairwise Compatibility:</h5>
                            ...
                            <div>
                                <h5 className="font-semibold text-lg text-blue-600">Group Harmony Score:</h5>
                                <p className="font-mono text-sm break-words">{report.math.harmonyEquation}</p>
                            </div>
                            </div>
                        </div>
                        )}
